...
const Navbar = () => {
  ...
  const onSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (q.trim()) nav(`/posts?q=${encodeURIComponent(q.trim())}`);
  };
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "glass-card shadow-card" : "bg-background/40 backdrop-blur-sm"}`}>
      <div className="container mx-auto grid grid-cols-[1fr_auto_1fr] items-center gap-4 px-6 py-3">
        ...
        <form onSubmit={onSearch} className="hidden md:flex justify-end">
      ...
      {open && (
        <div className="lg:hidden glass-card border-t border-border px-6 pb-4">
          ...
          <form onSubmit={onSearch} className="pt-3">
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search site..." className="w-full px-3 py-2 text-sm bg-card border border-border rounded-lg focus:outline-none focus:border-primary" />
          </form>
        </div>
      )}
    </nav>
